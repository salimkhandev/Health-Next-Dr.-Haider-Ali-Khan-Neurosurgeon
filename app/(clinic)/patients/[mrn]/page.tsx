    ...
    <div className="card p-5 hover:shadow-md transition-shadow">
      ...
      {visit.vitals && (
        <div className="flex flex-wrap gap-2 mb-3">
          {visit.vitals.bp && <span className="tag">BP: {visit.vitals.bp}</span>}
          {visit.vitals.pulse && <span className="tag">Pulse: {visit.vitals.pulse} bpm</span>}
          {visit.vitals.temperature && <span className="tag">Temp: {visit.vitals.temperature}°C</span>}
          {visit.vitals.weight && <span className="tag">Wt: {visit.vitals.weight} kg</span>}
        </div>
      )}
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="card p-6">
        <div className="flex flex-col sm:flex-row sm:items-start gap-5">
          {/* Avatar */}
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center shrink-0 text-3xl font-bold text-blue-600 border-2 border-blue-200">
            {fullName.charAt(0)}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {allergies?.length > 0 && (
              <div className="mb-2">
                <p className="text-[10px] font-bold uppercase text-red-400 tracking-wider mb-1 flex items-center gap-1">
            ...
            {chronicConditions?.length > 0 && (
              <div>
                <p className="text-[10px] font-bold uppercase text-amber-500 tracking-wider mb-1 flex items-center gap-1">
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            <div className="text-center">
              <div className="text-3xl font-extrabold text-blue-600">{visitCount}</div>
              <div className="text-xs text-slate-400 font-medium">Total Visits</div>
            </div>
            <div className="flex flex-col gap-2 w-full sm:w-auto mt-2">
              <Link href={`/consultation?mrn=${mrn}`} className="btn-primary text-xs justify-center">
                <FiPlus /> New Consultation
              </Link>
              <Link href={`/patients/${mrn}/export`} className="btn-secondary text-xs justify-center">
                <FiDownload /> Export Full History
              </Link>
              <button className="btn-ghost text-xs justify-center border border-slate-200 rounded-lg">
                <FiShare2 /> Share Link
              </button>
            </div>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <Link href={`/consultation?mrn=${mrn}`} className="btn-primary text-xs">
            <FiPlus /> New Visit
          </Link>
        </div>
        {visitDocs.length === 0 ? (
          <div className="card py-16 flex flex-col items-center text-center">
            <FaProcedures className="text-5xl text-slate-200 mb-3" />
            <p className="text-slate-500 font-semibold">No visits recorded yet.</p>
