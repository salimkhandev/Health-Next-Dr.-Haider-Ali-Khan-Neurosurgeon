...
export default function DashboardPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="card p-6 bg-gradient-to-r from-slate-900 via-slate-800 to-blue-950 text-white shadow-xl relative overflow-hidden">
        {/* Subtle background graphic */}
        <div
          className="absolute inset-0 opacity-10 pointer-events-none"
          style={{
            backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='%23fff' fill-opacity='0.2'%3E%3Cpath d='M30 0l30 30-30 30L0 30z'/%3E%3C/g%3E%3C/svg%3E")`,
          }}
        />

        <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start justify-between gap-6">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
            <div className="relative w-24 h-24 rounded-full overflow-hidden border-4 border-blue-500 shrink-0 shadow-lg bg-slate-800">
              <Image
            ...
            <div className="space-y-1">
              <div className="inline-block bg-gradient-to-r from-blue-600 to-red-500 text-white text-[10px] font-extrabold px-2.5 py-0.5 rounded-md uppercase tracking-widest shadow-xs">
              ...
              {settings?.specializationsList && settings.specializationsList.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-2">
                  {settings.specializationsList.map((spec, i) => (
                    <span
                      key={i}
                      className="bg-slate-800/80 text-blue-200 border border-slate-700 text-[10px] font-semibold px-2 py-0.5 rounded-full"
                    >
                      {spec}
                    </span>
                  ))}
          ...
          <div className="flex flex-col gap-2 shrink-0 w-full sm:w-auto">
            <Link href="/consultation" className="btn-primary justify-center shadow-md py-2.5">
              <FiPlus /> New Consultation
            </Link>
            <Link href="/patients" className="btn-secondary justify-center text-xs py-2 bg-white/10 hover:bg-white/20 text-white border-slate-700">
              <FaUserInjured /> Patient Directory
            </Link>
            <Link href="/wards" className="btn-secondary justify-center text-xs py-2 bg-white/10 hover:bg-white/20 text-white border-slate-700">
              <FaProcedures /> Ward Management
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-5 flex items-center justify-between border-l-4 border-l-blue-600">
          <div>
            ...
            <p className="text-[11px] text-slate-500 mt-0.5">Registered MRN records</p>
          </div>
        ...
        <div className="card p-5 flex items-center justify-between border-l-4 border-l-emerald-600">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Today&apos;s Visits</p>
            <p className="text-2xl font-extrabold text-slate-900 mt-1">{kpis.todayVisits}</p>
            <p className="text-[11px] text-slate-500 mt-0.5">Consultations logged today</p>
          </div>
        ...
        <div className="card p-5 flex items-center justify-between border-l-4 border-l-purple-600">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Ward Admissions</p>
            <p className="text-2xl font-extrabold text-slate-900 mt-1">{kpis.activeAdmissions}</p>
            <p className="text-[11px] text-slate-500 mt-0.5">Patients currently in beds</p>
          </div>
        ...
        <div className="card p-5 flex items-center justify-between border-l-4 border-l-red-600">
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Bed Occupancy</p>
            <p className="text-2xl font-extrabold text-slate-900 mt-1">{kpis.overallOccupancyRate}%</p>
            <p className="text-[11px] text-slate-500 mt-0.5">
              {kpis.occupiedBeds} of {kpis.totalBeds} beds occupied
            </p>
          </div>
      ...
      {/* 3. Follow-ups Due This Week Widget (FR-10.2) */}
      <div className="card p-5 space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2">
          <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider flex items-center gap-2">
            <FaCalendarAlt className="text-amber-500" /> Follow-ups Due This Week
          </h2>
